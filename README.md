...
Firebase 환경 변수가 없으면 로컬 데모 저장소로 실행됩니다. 실제 Google 로그인과 Firestore를 쓰려면 `.env.example`을 참고해 `.env.local`을 만드세요.
...
- Repository Variables: `FIREBASE_PROJECT_ID`, `VITE_FIREBASE_API_KEY`, `VITE_FIREBASE_AUTH_DOMAIN`, `VITE_FIREBASE_PROJECT_ID`, `VITE_FIREBASE_STORAGE_BUCKET`, `VITE_FIREBASE_MESSAGING_SENDER_ID`, `VITE_FIREBASE_APP_ID`
