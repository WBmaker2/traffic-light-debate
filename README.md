...
Firebase 환경 변수가 없으면 로컬 데모 저장소로 실행됩니다. 실제 Google 로그인과 Firestore를 쓰려면 `.env.example`을 참고해 `.env.local`을 만드세요. DB 백업 기록과 복원 목록은 Spark 무료 요금제에서도 동작하도록 Firestore `backups` 컬렉션에 저장합니다.
...
- Repository Secret: `FIREBASE_SERVICE_ACCOUNT_TRAFFIC_LIGHT_DEBATE`

`FIREBASE_SERVICE_ACCOUNT_TRAFFIC_LIGHT_DEBATE`는 Firebase CLI의 `firebase init hosting:github` 또는 Firebase Console의 서비스 계정 키로 만들 수 있습니다. `main` 브랜치에 push하면 live 채널, PR에는 preview 채널로 배포됩니다.

Firestore Rules까지 자동 배포하려면 이 서비스 계정에 Firebase Hosting 배포 권한 외에 Firebase Rules Admin 권한도 필요합니다. 권한이 없으면 Hosting 배포는 완료되고, Actions 로그에 Firestore Rules 권한 경고가 표시됩니다.
...
- PDF 저장
- 개별 세션 JSON 저장/불러오기
- 교사 소유 세션 Firestore DB 백업, 백업 기록 조회, JSON 복원
